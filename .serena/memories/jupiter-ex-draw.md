...
- WM/model **draw bind closed**.
- **Fat HUD (capture)**: Intro HUD = 2 fade-квада (white 4×4, орто-VS 0xbce3358 1/640+1/360, PS 0xbce3288, SrcAlpha/InvSrcAlpha); реальные HUD-элементы в Intro не рисуются (item IDs на боевом кадре). Bink: 0 вызовов в world capture (FEAR_Fettel.bik не в кадре).
- Зеркала: ReflectGroup = 5 RT Intro (Car_RT, AlmaFloor01_RT01, 09_RT01 non-mirror, Floor_RT, RenderTarget01), Med 256²; сессия = 2 камеры/кадр (VIEW -29200,-2235,-3480 и -2500,-1036,3219), last-writer = вторая.
- **R20 fog CLOSED**: table fog cvars (FogEnable/NearZ/FarZ + FogColor из WorldProperties → D3DRS 0x1c/0x22/0x24/0x25) из 0x004ffee0; FogEnable=1 на Aftermath/Docks/Vault; FogVolume-объектов в SP нет; PolyGrid = вода (Vault 2 шт, CPolyGridFX custom-render).
- **R30 CLOSED**: ModelDecal = CGameModelDecalMgr + skeletal multiply.fx (tex2Dproj mCustomTransform, Zero/SrcColor); CDecalFX world = custom-render; CustomRender контракт 0x0050f9b0 → ILTCustomRenderCallback.
- **R15 skin PS из capture** (0xb3af2e8, skin_ps_trace.txt): 7×2D, TBN из N, mul emissive*diffuse*spec*spec2*skin*blood.
- **Contradiction audit**: 517 DIP/555402 tris/3 sky DIP/1 Clear ALL ✓ (док 01-frame верен).
- **R27 Factory trace получен** (`fear-world.trace`, 495MB): 3820 Present, 803K DIP, меши до 20685v, 39653 текстур. Жирный кадр 529 DIP: 4× Clear ALL (RT: зеркало+RTG_Global) + Clear STENCIL ×12 — тот же паттерн R03/R05. Скрипт `scripts/fear-capture-world.sh`: дамп `--calls=2000000-`, детект DIP >5000v, запись 45с, требует нажатия клавиши на «Press Any Key». WTF_Entry — опционально тем же скриптом. `vt+0x40` @ `0x00478209` = interface slot (CWorldClientBSP vtable only 14 methods). Impl LoadSector `0x00478140`. Load `0x004782c0` → Shared this+4. World00p tail: header 119,66,12 + **66×36** AABB+surfIndex (empirical Intro); `347×208` rejected; 89660 B open. Still: who fills DAT_00576ff4+0x18 / object+0xf0; exe reader of 66×36/89660; 0x28 record bytes vs `0x00503a30`.
- BlackLight CLOSED: техника id 7 только в exe, контента нет (Light_BlackLight=0, ни одного .fx/Mat00/света) — мёртвая ветка.
- dx9lights.fxh/skin.fx/cloth_detail.fx: исходников в extract нет, но все skeletal .fxo на месте (skin.fxo = specular-клон + tSkinMap/tBloodMap/tSpecularMap2) — gap не блокирует.
...
- World00p tail CLOSED: 0x0050d0a0 reads TOC then TOC[0] entries (0x0050cf40). Entry0 nPieces=nSectors; others 1. TOC[1]=piece count. Each piece: nRec×36 AABB (Σ nRec=surf) + extra (u8 n + n×vec3). TOC[7]=extra polys, TOC[8]=Σ verts. 3 maps consume 100%. **TOC[9] читается и игнорируется (зарезервирован)**. **Extra = sky-порталы**: 0x00513ac0 клипит плоскостью+AABB, 0x005187d0 собирает AABB мира для sky-pass, 0x00515af0 debug-рисует цветом 0x50ffff00 под cvar DrawSkyPortals; в кадр не входят. Кусок 0x14: +0 extra ptr, +4 count, +0xc records, +0x10 count.
...
- R14 leftover: skeletal.fxh absent (макросы в .fxo); type 4 без семпла (0 в Arch00). type 3/7 third float = **density_g** (SDK HandleSphere/CreateCapsuleShape) CLOSED; 199v двор = **skeletal specular_env** (capture: stride 64, 5 сэмплеров, env cube 64² DXT3) CLOSED.
- R15 skin: **PS Point из capture** (`0xb3af2e8`, `skin_ps_trace.txt`): 7×2D, TBN из N (как hair), mul-цепь emissive*diffuse*spec*spec2*skin*blood, pow(spec.a). Остаток: сэмплер s3/s4/s5 ↔ skin/blood/glancing по CTAB skin.fxo.
